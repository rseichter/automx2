...
<1> If this environment variable exists, it will be used. The value must point
    to a location where automx2 can read configuration from. It will proceed if
...
====
See Mozilla's
link:https://wiki.mozilla.org/Thunderbird:Autoconfiguration:ConfigFileFormat#Placeholders[Thunderbird:Autoconfiguration:ConfigFileFormat]
for a documentation of available placeholders like `%EMAILLOCALPART%`.
====
