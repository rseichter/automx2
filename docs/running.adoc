...
While logged in as an unprivileged user, change into the installation directory and start the `.venv/bin/flask` launch script:

[source,bash]
----
cd /srv/www/automx2
FLASK_APP=automx2.server:app .venv/bin/flask run --host=127.0.0.1 --port=4243
----
